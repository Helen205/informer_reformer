...
def plot_figure(path, historty, args):
    ...
    x_ticks_minor[0] = 0
    y_ticks_1_major = np.arange(0, y1_limit, 0.05) 
    y_ticks_1_minor = np.arange(0, y1_limit, 1)

    y0_limit =0.0001
    y_ticks_0_major = np.arange(0, y0_limit, 1e-5)
    y_ticks_0_minor = np.arange(0, y0_limit,  1)
    ...
    axs[0].set_ylim(0, y0_limit)
    
    axs[0].set_yticks(y_ticks_0_major)
    axs[0].set_yticks(y_ticks_0_minor, minor=True)
    axs[0].grid()
